...
declare
fun {COP Y} %Comparative Operator
    Y=='<' orelse Y=='>' orelse Y=='=<' orelse
    Y=='>=' orelse Y=='==' orelse Y=='!='
end
    
declare
fun {EOP Y} Y=='+' orelse Y=='-' end %Expression + or - operator

declare
fun {TOP Y} Y=='*' orelse Y=='/' end %* or / operator

fun {IsIdent X} {IsAtom X} end %Isident / IsAtom
...
declare Sn
{Browse{Prog[program foo ';' while a '+' 3 '<' b 'do' b ':=' b '+' 1 'end'] Sn}}
